...
- **compliance-review** — 合规风险缺口作为风险评估输入，识别行政和监管风险维度
- **company-background-check** — 交易对手背调结果作为风险线索，支撑履约能力评估
- **case-lookup** — 诉讼风险引用类案裁判趋势，量化败诉可能性和损失
- **contract-management/contract-review** — 合同条款风险以合同审查结果为输入（详见跨包引用）
- **corporate-tax/financial-analysis** — 财务风险量化支撑，评估交易和税务风险敞口

## 失败回退机制

| 步骤 | 失败条件 | 回退目标 | 最大重试 | 不可恢复时升级路径 |
|------|---------|---------|---------|------------------|
| 第一步：场景识别与风险扫描 | 评估对象描述过于模糊，无法匹配高频场景或识别风险维度 | 默认按"通用评估+六大维度全扫描"执行，标注"[场景未匹配，按通用评估执行]" | 2次 | 建议用户补充业务背景（业务模式/合同关系/交易结构）后再扫描 |
| 第二步：风险评估 | 已识别风险缺少足够信息判定可能性或严重度 | 默认按"可能性=中，严重度=中等"（三级黄色）处理，标注"[评估依据不足，等级为默认值]" | 2次 | 建议用户补充风险线索和历史数据后重新评估 |
| 第三步：制定应对方案 | 二级以上风险的应对方案缺乏具体操作细节 | 输出应对策略方向，标注"[具体行动待补充，建议咨询专业律师制定详细方案]" | 1次 | 建议用户委托专业律师出具正式风险应对方案，尤其是重大交易和合规整改场景 |
| 第四步：风险监控建议 | Notion 连接失败或监控指标无法量化 | 降级为输出风险监控清单（Markdown），指标栏标注"[待量化，建议与业务部门协商确定]" | 2次 | 输出 Markdown 文件并提示用户检查 Notion 连接器配置，监控指标建议与业务部门协商 |
