...
- **risk-assessment** — 合规缺口转化为法律风险矩阵评估，量化风险等级
- **legal-daily-briefing** — 合规整改进展纳入法务日报跟踪，持续监控完成状态
- **corporate-tax** — 税务合规事项需财税数据支撑，覆盖发票和纳税申报合规
- **contract-management/contract-review** — 格式条款和合同合规审查互补，覆盖交易文件合规

## 失败回退机制

| 步骤 | 失败条件 | 回退目标 | 最大重试 | 不可恢复时升级路径 |
|------|---------|---------|---------|------------------|
| 第一步：识别合规领域和行业分支 | 业务描述过于模糊，无法识别行业或涉及合规领域 | 默认按"综合审查+全领域扫描"执行，标注"[行业未识别，按综合审查执行]" | 2次 | 建议用户补充业务类型、运营模式、目标市场后再审查 |
| 第二步：逐领域审查 | 法规时效性存疑（疑似已修订/废止）或跨境法规适用性不明 | 标注"[法规时效性待核实，建议通过国家网信办/市场监管总局官网验证]" | 3次 | 建议用户咨询专业合规律师确认最新法规要求，特别是跨境合规领域 |
| 第三步：量化评分 | 审查项信息不足，无法判定合规/基本合规/不合规 | 默认按"基本合规（1分）"处理，标注"[评分依据不足，建议补充信息后重新判定]" | 2次 | 输出待判定清单，建议用户补充业务细节后重新评分 |
| 第四步：生成整改清单 | P0级整改项缺乏具体操作步骤或验收标准 | 输出整改方向，标注"[具体操作步骤待补充，建议咨询专业合规顾问]" | 1次 | 建议用户委托专业合规顾问制定详细整改方案，尤其是跨境合规和数据出境事项 |
