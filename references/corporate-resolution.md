...
- **legal-documents** — 决议相关法律文书（如股东会通知、决议公告）起草互补
- **e-signature** — 决议文件和章程修正案的电子签署流程发起
- **company-background-check** — 股东和实控人信息核查，确认表决权和关联关系
- **contract-management/contract-drafting** — 股权转让协议等配套交易文件起草

## 失败回退机制

| 步骤 | 失败条件 | 回退目标 | 最大重试 | 不可恢复时升级路径 |
|------|---------|---------|---------|------------------|
| 第一步：判断决议类型和表决要求 | 公司类型或决议事项不明确，无法确定表决比例 | 默认按"有限责任公司+股东会特别决议（2/3以上）"处理，标注"[决议类型待用户确认]" | 2次 | 建议用户明确公司类型和决议事项后重新判断，或咨询法务确认 |
| 第二步：检查程序合规性 | 章程特殊约定未知，无法判断是否有不同于法定的表决比例 | 默认按法定比例执行，标注"[未确认章程特殊约定，按法定比例执行，建议核实章程]" | 1次 | 建议用户提供公司章程后重新检查程序合规性 |
| 第三步：生成决议文件 | 股权结构或出席情况不明，无法填写表决结果 | 表决结果栏标注【待确认：各股东表决权比例】，决议文件其余部分正常输出 | 2次 | 输出决议文件模板，建议用户补充股权结构和出席情况后完善 |
| 第四步：生成配套章程修正案（如适用） | 决议事项不涉及章程修改或修改内容不明 | 跳过章程修正案生成，标注"[本次决议无需章程修正案]"或"[修改内容待确认]" | 1次 | 建议用户明确需修改的章程条款后重新生成 |
| 第五步：提示后续工商变更 | 决议事项不涉及工商变更或变更事项不明 | 标注"[本次决议无工商变更事项]"或"[变更事项待确认，建议咨询工商代理]" | 1次 | 建议用户咨询工商代理机构确认变更需求 |
| 第六步：决议归档 | Notion 连接失败或权限不足 | 降级为 Markdown 直接输出完整决议文件和章程修正案，提示用户手动保存 | 2次 | 输出 Markdown 文件并提示用户检查 Notion 连接器配置 |
